...
from networks.DPCRN import DPRNN, DPCRN_NET, DPCRN_multi
...
if __name__ == "__main__":
    ...
    # modelpath = 'model_dpcrn_multi/'#极角和方位角反了
    modelpath = 'model_dpcrn_multi_position/'  # 正确的极角和方位角
    ...
    for epoch in range(args.num_epoch):
        ...
        for idx, egs in tqdm(enumerate(train_loader), total=len(train_loader)):
            # B X C X T
            input = egs['input'].cuda()
            target = egs['target'][:, 0, :].cuda()

            input_1 = egs['input_1'].cuda()
            target_1 = egs['target_1'].cuda()
            input_2 = egs['input_2'].cuda()
            target_2 = egs['target_2'].cuda()
            input_3 = egs['input_3'].cuda()
            target_3 = egs['target_3'].cuda()
            input_4 = egs['input_4'].cuda()
            target_4 = egs['target_4'].cuda()

            out_1, out_2, out_3, out_4 = network(input_1, input_2, input_3, input_4)

            # compute loss
            loss_1 = loss_function(out_1, target_1)
            loss_2 = loss_function(out_2, target_2)
            loss_3 = loss_function(out_3, target_3)
            loss_4 = loss_function(out_4, target_4)
        ...
        for idx, egs in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            loss_3 = loss_function(out_3, target_3)
            loss_4 = loss_function(out_4, target_4)
            loss_val = loss_1 + loss_2 + loss_3 + loss_4
